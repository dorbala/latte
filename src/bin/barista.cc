...
#include "feat/splice-feats.h"
#include "feat/transform-feats.h"
...
bool SpawnActors(adjacency_list_type& graph, ptree& config,
  ...
  for (int u = 0; u < graph.size(); u++) {
    ...
    } else if (actor_type == "CompareWords") {
      actor_map->insert(make_pair(actor_name, spawn<CompareWords>(actor_name, subscribers, config)));
    ...
    } else if (actor_type == "SpliceFeats") {
      actor_map->insert(make_pair(actor_name, spawn<SpliceFeats>(actor_name, subscribers, config)));
    } else if (actor_type == "TransformFeats") {
      actor_map->insert(make_pair(actor_name, spawn<TransformFeats>(actor_name, subscribers, config)));
