...
#include "../io/namespace_test.h"

using namespace std;
using namespace kaldi;
using namespace cppa;
int bar::decoded_words = 0;
...
void WriteBestPath(FasterOnlineDecoder &decoder,
	...
	std::ofstream fh;
	fh.open(decode_filename,std::ios_base::app);
	fh << "Found: ";
        if (word_syms != NULL) {
            std::cerr << key << ' ';
    ...
    bar::decoded_words+=words_size;

    cout << "Received word size: " << bar::decoded_words << endl;
    //return words_size;
}
    ...
    on(atom("UTT_END")) >> [=]() {
      ...
      if (utt_frame_count > 0) {
          WriteBestPath(*decoder, allow_partial, acoustic_scale, utt_key, 
                        ...
                        &num_success, &num_fail, &frame_count, decode_filename, &tot_like);
          utt_frame_count = 0;
      }
      utt_number++;
    },
